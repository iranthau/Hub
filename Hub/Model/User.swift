...
class User: PFUser {
    ...
    func hasContactsToShar() -> Bool {
        return self.objectForKey("contacts") != nil
    }
    ...
    class func logIn(userDetails: [String: String], completion: (success: User?, error: String?) -> Void) {
        ...
        self.logInWithUsernameInBackground(username, password: password) {
            ...
            if let error = error {
                ...
                completion(success: user, error: nil)
            }
        }
    }
                ...
                if pUser.isNew {
                    HubAPI.readFacebookPublicProfile {
                        ...
                        if let error = error {
                            ...
                            let facebookUser = pUser as? User
